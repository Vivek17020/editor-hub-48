...
serve(async (req) => {
  ...
  try {
    ...
    const { priceId, planName, planPrice } = await req.json();
    logStep("Request data parsed", { priceId, planName, planPrice });
      ...
      line_items: [
        {
          price_data: {
            ...
            recurring: { interval: planName.includes('Pro') ? 'year' : 'month' },
          },
          quantity: 1,
        },
      ],
